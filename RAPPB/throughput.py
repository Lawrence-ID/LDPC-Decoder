...
for code_rate_ncalc, ax in zip(code_rate_list, [ax1, ax2]):
    ...
    # 新增：绘制折线图
    ax.plot(all_zc, throughput_values, marker='o', linestyle='-', label=f'Proposed (Avg: {avg_throughput:.2f} Gbps)', markersize=3)
    ax.plot(all_zc, xilinx_throughput_values, marker='x', linestyle='--', label=f'Xilinx (Avg: {avg_xilinx_throughput:.2f} Gbps)', markersize=3)
