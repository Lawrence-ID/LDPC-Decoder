...
class FoldedSRAMTemplate[T <: Data](
  ...
  val array = Module(new SRAMTemplate(
    ...
    separateGateClock = separateGateClock
  ))
  ...
  val use_conflict_data =
    conflict_valid && conflict_addr === RegEnable(raddr, io.r.req.valid) && conflict_widthIdx === ridx
